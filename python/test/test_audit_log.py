import responses
import uuid
from audit_middleware import Auditor, AuditLogger, GatewayHost, TraceId
...
def test_when_calling_enhance():
    ...
    r = logger.enhance(TraceId(test_trace_id), "this is a message")
    r is None
...
def test_subsequent_messages_have_fresh_context():
    ...
    logger.message().append("key-1", "foo").log(test_trace_id)
    logger.message().append("key-1", "bar").log(test_trace_id)
...
def test_can_log_uuid():
    ...
    _uuid = uuid.uuid4()
    (
        AuditLogger(GatewayHost(GATEWAY_HOST))
        .message()
        .append("uuid", _uuid)
        .log(test_trace_id)
    )
